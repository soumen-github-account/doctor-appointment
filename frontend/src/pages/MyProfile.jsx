  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      
      {
        ...
        : <img className='w-36 rounded-sm' src={userData.image} alt="" />
      }

      {
        isEdit ? <input className='bg-gray-50 text-3xl font-medium mt-4' type="text" value={userData.name} onChange={e => setUserData(prev => ({...prev, name:e.target.value}))} />
      ...
      <div className='mt-10'>
        {
          isEdit
          ?  <button className='border border-[#5f6FFF] px-8 py-2 rounded-full hover:bg-[#5f6FFF] hover:text-white transition-all cursor-pointer' onClick={updateUserProfileData}>Save Information</button>
